 ...
 * @since  1.0.0
 *
 * @return {Promise} - returns a Promise, resolves to a string
...
async function createContractWithAsset(token, clientAppName, primeArtifactName) {
  const appId = await getApplicationId(token, clientAppName);
  const apiId = await getApiId(token, primeArtifactName);
  const versionId = await getVersionId(token, apiId, 'v1');
  const contract = await createContractRequestingAccess(token, apiId, versionId, appId);
  return contract;
}
...
module.exports = {
  ...
  getExchangeGroups,
  createContractWithAsset,
  isClientApplicationByName,
};
